...
- 类加载器通过全限定名去加载类，如果是数组类型，还会由JVM生成一个代表此数组维度和元素的数组对象 
- 最后，检验是否有相应的访问权限

#### 字段解析

- 在当前类中查找到简单名称和字段描述符都与目标相匹配的字段，则返回字段的直接引用
- 否则，按照继承关系，从下往上递归搜索各个接口是否有匹配
- 否则，如果不是Object类，继续从下往上搜索其父类是否有匹配
- 否则，查找失败
- 查找成功后，会进行权限验证
...
- 准备阶段，变量被赋予系统默认的初始值；初始化阶段，则根据代码去初始化类变量和其他资源

#### \<clinit\>

- 由编译器自动收集类中的所有类变量的赋值动作和静态语句块中的语句合并产生；顺序是由在源文件中出现的顺序决定；静态语句块只能访问到定义在静态语句块之前的变量，定义在它之后的变量，可以赋值，但不能访问
- \<clinit\>之前不会显示地调用父类的\<clinit\>，虚拟机会保证子类\<clinit\>执行之前，父类的\<clinit\>已经执行完毕
- 只有有静态语句块和静态变量赋值操作才能产生<clinit\>方法
- 同时只有一个线程能够执行<clinit\>方法，并且只会执行一次
- 接口的<clinit\>只有在接口中定义的变量被调用时才能执行

## 类加载器

- 每一个类加载器，都拥有一个独立的类名称空间
- 同一个类被不同的类加载器加载，这两个类就必然不相等

### 启动类加载器

- 程序员不能直接使用
- 加载lib目录下，或者被-Xbootclasspath参数所指定的路径中，且被虚拟机识别的类

### 拓展类加载器

- 加载lib\ext目录中或者java.ext.dirs系统变量锁指定的路径中的类库
...
- 物理机：执行引擎是建立在处理器、硬件、指令集和操作系统层面上
- 虚拟机：执行引擎是由自己实现，可以指定指令集与执行引擎的结构体系，并且能够执行哪些不被硬件直接支持的指令集格式

## 运行时栈帧结构

- 每一个方法调用从开始到执行完成，对应着一个栈帧在虚拟机栈中的入栈和出栈过程
- 栈帧包括局部变量表、操作数栈、动态连接、方法返回地址和附加信息

### 局部变量表

- 局部变量表是一组变量值存储空间，用于存放方法参数和方法内部定义的局部变量
- 32位数据类型占用1个slot，64位数据类型占用2个slot（并采用高位对齐）
- 执行实例方法，局部变量表0号索引是this指针
- 方法执行超过某个变量的作用域后，该变量的slot可以被重用；这样该变量暂时不会被gc

### 操作数栈

- 32位数据类型所占栈容量为1,64位数据类型所占栈容量为2

### 动态连接

- 动态连接：在每一次运行的时候，将符号引用转换为直接引用
- 静态解析：在类加载阶段或第一次使用时，就转化为直接引用

### 方法返回地址和附加信息

- 正常退出的时候的返回地址保存在栈帧中
- 附加信息：虚拟机允许实现增加一些规范里没有描述的信息到栈帧中，例如与调试相关的信息
...
- 所有依赖静态类型来定位方法执行版本的分派动作称为静态分派，例如方法重载
- 方法重载没有合适的类型的时候，会寻找更加合适的版本
- 基本类型：从小到大——包装类——父类——变长参数
...
- 宗量：方法的接受着与方法的参数统称为方法的宗量
- 静态分派属于多分派
- 动态分派属于单分派

#### 动态分派的实现

- 虚方法表存放着类的各个方法的实际入口地址，子类覆盖父类的方法，那么父类的该方法的实际入口地址指向子类
- 方法表一般在类加载的连接阶段进行初始化
- 如果类的方法在方法表中的位置是固定的，接口由于多实现的关系，并不固定；

## 基于栈的字节码解释引擎

- 栈的指令集具有可移植性，用户程序不会直接使用寄存器
- 但是速度会慢一些

# 程序编译与代码优化

- 前端编译器：把*.java文件转变成*.class文件
- JIT编译器：运行时把字节码编译成机器码
- AOT编译器：直接把源代码编译成机器码
...
- Client Compiler编译速度更快
- Server Compiler编译的质量更好，但耗时更长

### 分层编译

1. 程序解释执行，解释器不开启性能监控功能，可触发第一层编译
2. C1编译，将字节码编译为本地代码，进行简单可靠的优化，如有必要将加入性能监控的逻辑
3. C2编译，将字节码编译为本地代码，启动一些编译耗时较长的优化，甚至会根据性能监控信息进行一些激进的优化
